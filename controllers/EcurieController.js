...
module.exports.GetEcurie = function(request, response) {
  ...
  async.parallel([
    function(callback){
      model.get(id, function(err, res) { callback(err, res) });
    }, //Fin callback 0
    function (callback) {
      model.getListeEcurie( function (err, result) { callback(err, result) });
    },
    function (callback) {
      model.getPilote(id, function (err, res) { callback(err,res)});
    },
    function(callback) {
      model.getVoiture(id, function(err, res) { callback(err,res)});
    }
  ],
  function (err, result) {
    if (err) {
